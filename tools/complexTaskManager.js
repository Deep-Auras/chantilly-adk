...
class ComplexTaskManagerTool extends BaseTool {
  constructor(context) {
    ...
    this.description = 'Create and manage complex INTERNAL multi-step tasks that require long execution times, progress tracking, and worker processes. Use for operations like financial reports, bulk data processing, comprehensive analysis, or multi-step workflows that execute CODE. Examples: "Generate Q4 report", "Analyze all invoices and create follow-up tasks", "Build project plan with milestones". CRITICAL: When user requests ANY kind of report, invoice report, or data analysis, you MUST ALWAYS use action="create" with auto-detection. DO NOT use for: (1) SIMPLE DIRECT task creation like "Create an Asana task called X" - use AsanaTaskManager instead, (2) Template management - use TaskTemplateManager tool, (3) Simple searches or single tool operations. NOTE: You CAN use this for complex workflows that INCLUDE creating external tasks as PART of the process (e.g., "Generate project plan AND create all tasks in Asana"). This tool executes JavaScript code that can call other services.';
